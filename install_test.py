...
install_template_test = '''
git clone https://github.com/Rainysponge/X.git --branch Y
cd X
cmake .
make -j4
sudo make install
...
for k in ['tool', 'sh2pc']:
	if vars(args)[k]:
		template = install_template_test.replace("X", "emp-"+k).replace("Y", vars(args)[k])
		print(template)
		subprocess.call(["bash", "-c", template])
